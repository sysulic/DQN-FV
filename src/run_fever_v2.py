...
def evaluate(args: dict, agent, save_dir: str, dev_data: FeverDataset=None, is_eval: bool=True):
    ...
    name = 'decision_seq_result_v2.json'
    if not is_eval:
        name = f'test-{name}'
    #if args.do_fever2:
    #    name = f'fever2-{name}'
    ...
    if not is_eval:
        ...
        predicted_list = calc_test_result(results, args.test_true_file, logger=None, is_test=True, is_precise=False)
        with open(os.path.join(save_dir, 'test-imprecise_v2.jsonl'), 'w') as fw:
            for item in predicted_list:
                fw.write(json.dumps(item) + '\n')
        
        results = truncate_q_values(results_of_q_state_seq, is_test=True, is_precise=True)
        predicted_list = calc_test_result(results, args.test_true_file, logger=None, is_test=True, is_precise=True)
        with open(os.path.join(save_dir, 'test-precise_v2.jsonl'), 'w') as fw:
            for item in predicted_list:
                fw.write(json.dumps(item) + '\n')

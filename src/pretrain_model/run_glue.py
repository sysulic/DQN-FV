...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        output_eval_file = os.path.join(eval_output_dir, prefix, f"eval_results-{eval_loss}.txt")
        with open(output_eval_file, "w") as fw:
            lines = list(map(lambda scores: ' '.join(map(str, scores)), preds.tolist()))
            fw.write('\n'.join(lines))
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    bert_type = args.config_name if args.config_name else args.model_name_or_path
    ...
    cached_features_file = os.path.join(args.data_dir, 'cached_{}_{}_{}_{}'.format(
        ...
        list(filter(None, bert_type.split('/'))).pop(),
